...
export default function Button({text, type, onClick, active}) {
    ...
    let pointerEvents = active ? 'cursor-pointer' : 'cursor-none';

    let className = `flex justify-center items-center gap-1 p-1.5 w-50 border-none rounded-xl font-bold ${bgColor} ${pointerEvents} shadow-grey-100 text-slate-100 hover:-translate-y-1 hover:shadow-[0_6px_16px_rgba(0,0,0,0.4)] active:shadow-[0_2px_6px_rgba(0,0,0,0.2)] transition-all`;

    if (type === 'continue'){
        return (
            <div className={className} onClick={onClick}>
                <h2 className='text-l'>{text}</h2>
